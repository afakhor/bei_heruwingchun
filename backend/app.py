...
def get_market_screener():
    try:
        ...
        raw_quotes = []
        chunk_size = 100  # Kita bagi aman per 100 saham sekali tembak
        
        # Looping untuk memecah daftar saham agar tidak kena limit URL
        for i in range(0, len(active_tickers), chunk_size):
            ...
            if response.status_code == 200:
                ...
                raw_quotes.extend(results)  # Gabungkan hasil ke list utama
        
        screener_list = []
        for q in raw_quotes:
            ticker_clean = q.get('symbol', '').replace('.JK', '')
            
            # Gunakan fallback 0 jika data dari Yahoo bernilai None (saham suspensi/tidak likuid)
            change_percent = float(q.get('regularMarketChangePercent') or 0)
            price = float(q.get('regularMarketPrice') or 0)
            volume = float(q.get('regularMarketVolume') or 0)
            
            # Logic Analisis Sederhana untuk Radar Fast Trade
            signal = "Neutral"
            if change_percent > 4.0 and volume > 1000000:
                signal = "💥 HIGH MOMENTUM / SPIKE"
            elif change_percent > 1.5:
                signal = "🟢 AKUMULASI INTRADAY"
            elif change_percent < -3.0:
                signal = "🚨 DISTRIBUSI / PANIC SELL"
